...
impl Topology {
    ...
    pub fn get_core_diff_power_microwatts(&self) -> Option<MultiValuedRecord> {
        ...
        debug!("Using formula v_active_ipc_aperf_aperf_div_mperf");
        let coefs = self.get_cores()
            .iter()
            .map(|c| {
                if let Some(metrics) = c.get_core_metrics_delta() {
                    if metrics.mperf > 0 {
                        (1_f64 + metrics.active_percentage * metrics.ipc) * (metrics.aperf as f64 * (metrics.aperf as f64 / metrics.mperf as f64))
                    } else {
                        0.0
                    }
                } else {
